...
class CityTableViewController: UITableViewController {
        ...
        { (data, Response, error) in
            ...
            else
            {
                if let content = data
                {
                    do
                    {
                        let myJSon = try JSONSerialization.jsonObject(with: content, options: JSONSerialization.ReadingOptions.mutableContainers) as! [String:[String:AnyObject]]
                        let temp = myJSon["result"]
                        self.dataArray = temp!["results"] as! [AnyObject]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! CityTableViewCell
            ...
            cell.cityName?.text = dataArray[indexPath.row]["locationName"] as? String
